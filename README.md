...
- [Architecture & Security Features](#architecture--security-features)
  - [Login & Timing Attack Prevention](#login--timing-attack-prevention)
...
- **Sliding Session Expiration:**
  Implements a sliding expiration policy for sessions. Everytime a token is renewed, the expiration time is reset. Making the session seamless to the user.
...
- **Timing Attack Prevention:**
  The login process utilizes constant-time comparison strategies. If a user is not found, the system still performs a password verification against a dummy hash. This ensures that the response time for "User Not Found" and "Incorrect Password" is indistinguishable, preventing attackers from enumerating valid usernames based on response latency.
...
<details>
  <summary>Click to view Login Sequence Diagram</summary>
  ...
  sequenceDiagram
    ...
    alt User Found
        ...
        Note over API, HashService: CRITICAL: Execute Dummy Verify<br/>to simulate processing time
...
<details>
  ...
  sequenceDiagram
    ...
    rect rgb(168, 66, 64)
        ...
        Note over API: Optimization: Skip DB Write.<br/>Reuse Token B generated by Process 1.
...
```bash
git clone https://github.com/pedroheing/cookie-auth.git
cd cookie-auth
```
...
| **Redis Insight** | `http://localhost:5540` | N/A |

### Database Management

To access the database via pgAdmin:

1. Access `http://localhost:5050` in your browser.
2. Login to pgAdmin (`admin@admin.com` / `root`).
3. Add a new server using the hostname `postgres` (internal Docker network alias).
4. Use the username `admin` and password `password`.

### Redis Management

To access the Redis Insignt:

1. Access `http://localhost:5540` in your browser.
2. Add the following connection string: `redis://default@redis:6379`
